...
public class IsodataClustering {
	...
	private int mergeClusters(List<Cluster> clusters) {
		...
		for(int i = 0; i < clusters.size(); i++) {
			...
			for(int j = i + 1; j < clusters.size(); j++) {
				...
				float distance = Vector3f.sub(c1.center, c2.center, new Vector3f()).length();
	...
	private int splitClusters(List<Cluster> clusters, float clusterMeanDistance) {
		...
		for(Split split : splits) {
			...
			c1.setCenter(Vector3f.add(split.cluster.center, offset, new Vector3f()));
			c2.setCenter(Vector3f.sub(split.cluster.center, offset, new Vector3f()));
	...
	private Cluster findClosestCluster(List<Cluster> clusters, Vector3f sample) {
		...
		for(Cluster cluster : clusters) {
			float sqDst = Vector3f.sub(sample, cluster.center, new Vector3f()).lengthSquared();
