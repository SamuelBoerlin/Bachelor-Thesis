...
The goal of this chapter is to go into more detail about the previously mentioned concepts and how they
...
several changes had to be made to Cineast. However due to Cineast's modular structure must of the changes could be applied without having to
...
The \textsc{OBJMeshDecoder}, which is responsible for reading a .obj model file into a \textsc{Mesh}, was extended to also read texture UV coordinates from the file.
...
\textsc{persist(SegmentContainer, MediaSegmentDescriptor)} which is called when a \textsc{SegmentContainer} is being persisted by \textsc{GenericExtractionItemHandler}.
...
A similar change was required in \textsc{ModelQueryContainer}. Model data is being sent to Cineast in the form of a custom string based model format. This format however does not support vertex colors which are required for color sensitive queries. For this reason a new \textsc{ColorMeshParser} class was added that uses a similar format but also supports vertex colors. Based on the MIME type, an identifier used to describe the format of data, that prepends the model data string the \textsc{ModelQueryContainer} can decide which parser to use. Like this the API remains backwards compatible with the model format that does not contain vertex color data.
...
to generate an OpenAPI specification that describes Cineast's REST API in a standardized way like shown in Listing \ref{lst:openapi_spec}.\\
...
Listing \ref{lst:csharp_cineast_api_call} shows how to send Cineast a similarity query request by calling the Swagger Codegen generated C\# method \textsc{FindSegmentSimilarActionHandlerPOSTAsync}.
...
shaders has changed. Shaders are GPU programs that determine how objects in the scene are rendered to the screen, e.g. by computing lighting or adding special effects. Some of these shaders, most notably the highlight when grabbing an object, were required for our application and thus had to be partially rewritten to work in the new URP.
...
As mentioned before our application uses a texture array to store all the voxel material textures on the GPU for rendering. This texture array is created by the \textsc{VoxelTerrainTextures} script which is to be added to the same object that contains the \textsc{DefaultVoxelWorldContainer} component. On start of the application it reads all the textures that have been added to the \textsc{VoxelTerrainTextures} component's textures array into a texture array on the GPU and then assigns it to the "\_TextureArray" attribute of the object's material such that the tri-planar texturing shader can access it.
...
The controller hints shown in Fig. \ref{fig:controller_hints} help the user become familiar with the VR sculpting application. They are always shown when the application is started but can then be toggled off or back on by pressing a button on the controller.
...
The most important tool available to the user is the brush. It is attached to the right controller as shown in \ref{fig:editing_showcase} and displayed as a translucent preview of the brush's shape. Using a custom shader this brush preview shows the intersection between the brush and the sculpture. This feature has turned out to be an important addition, because without it it's quite difficult to tell where the brush intersects with the sculpture despite the stereoscopic display of the VR headset.\\
...
The brush selection menu in Fig. \ref{fig:gui_brush_select} lets the user select different brushes. It is arranged as a radial menu centered in front of the user's right controller. This the user quickly and conveniently select other brush types without having to move the hand much.\\
...
Fig. \ref{fig:query_pointing} shows the curved query results UI with the user pointing at a result object.

\begin{figure}
\centering
\captionsetup{width=0.8\textwidth}
\includegraphics[width=0.75\textwidth]{query_voxelized.png}
\caption{A voxelized model from the query results UI.}
